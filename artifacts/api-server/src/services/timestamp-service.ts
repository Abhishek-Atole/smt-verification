...
export class TimestampService {
  ...
  static async syncWithNTP(ntpServer: string = "pool.ntp.org"): Promise<boolean> {
    try {
      ...
      if (Math.abs(drift) > 1000) {
        ...
        logger.info({ drift, serverTime: serverTime.toISOString() }, "Timestamp service synchronized");
        return true;
      }

      this.isNTPSynced = true;
      logger.info("Timestamp service found no clock drift");
      return true;
    } catch (error) {
      logger.warn({ err: error }, "Timestamp service NTP sync failed");
      return false;
    }
  }
  ...
  static async initialize(): Promise<void> {
    logger.info("Initializing timestamp service");
    await this.syncWithNTP();
    logger.info({ currentTime: this.getCurrentTimestampISO() }, "Timestamp service ready");
  }
}

// Auto-initialize on import
TimestampService.initialize().catch((err) =>
  logger.error({ err }, "Timestamp service initialization error")
);
