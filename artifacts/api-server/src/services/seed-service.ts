    ...
    for (let b = 0; b < bomsPerCompany; b++) {
        ...
        if (!component) {
          try {
            ...
            const existing = (await db
              ...
              .where(eq(componentsTable.mpn, compData.mpn))) as any[];
      ...
      for (let s = 0; s < sessionsPerBom; s++) {
        ...
        if (Math.random() > 0.5 && bomItems.length > 2) {
          try {
            const randomBomItem = bomItems[Math.floor(Math.random() * bomItems.length)];
            const operatorId = randomUUID();
            const splice = (await db
              .insert(spliceRecordsTable)
              .values({
                changeoverId: session[0].id.toString(), // SMT ID as string
                lineItemId: randomUUID(),
                oldSpoolMpn: `OLD-MPN-${Math.floor(Math.random() * 100000)}`,
                oldSpoolLot: `OLD-LOT-${Math.floor(Math.random() * 1000)}`,
                newSpoolMpn: `NEW-MPN-${Math.floor(Math.random() * 100000)}`,
                newSpoolLot: `NEW-LOT-${Math.floor(Math.random() * 1000)}`,
                splicedBy: operatorId, // Use the operator ID from the session
