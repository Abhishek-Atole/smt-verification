...
export class ExportService {
   ...
   */
  /**
   * Record export in audit table with hashed PII (BUG-13 fix)
   * Hashes IP address and user-agent to protect privacy
   */
  static async recordExport(
    ...
    try {
      ...
      const hashedIpAddress = ipAddress ? await bcryptjs.hash(ipAddress, 10) : undefined;
      const hashedUserAgent = userAgent ? await bcryptjs.hash(userAgent, 10) : undefined;
  ...
  static deleteExportFile(filepath: string): void {
    try {
      ...
      logger.error({ err: error }, "Failed to delete export file");
    }
  }
}
